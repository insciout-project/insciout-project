...
4. the Meta table.
: contains the meta information for the Press Release and related Article and News.  It is the concatenation of the content from [A:B, 1:8] across all excel spreadsheets and all samples.
...
This division of the inforamtion into four tables will allow us to cross easily the information between Meta, PR, JA and News. To cross information is called a `MERGE` or `JOIN` operation, which is at the core of `SQL` and modern data manipulation libraries (R, Python, MATLAB)
...
A `view` as seen in the `SQL` tutorial is a virtual table that is generated from the actual table in the database. These virtual tables can be used as if there are actual table.
